...
def parse_test_shared(parser):
    ...
    parser.add_argument('-t', '--runtime', type=int, default=30, help='total runtime in seconds (default 30)')
    parser.add_argument('--interval', type=int, default=0, help='interval in seconds between two flows (default 0)')

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--all', action='store_true', help='test all schemes specified in src/config.yml')
    group.add_argument('--schemes', metavar='"SCHEME1 SCHEME2..."', help='test a space-separated list of schemes')

    parser.add_argument('--run-times', metavar='TIMES', type=int, default=1,
                        help='run times of each scheme (default 1)')
    parser.add_argument('--start-run-id', metavar='ID', type=int, default=1, help='run ID to start with')
    parser.add_argument('--random-order', action='store_true', help='test schemes in random order')
    parser.add_argument('--data-dir', metavar='DIR', default=Path(context.src_dir) / 'experiments' / 'data',
                        help='directory to save all test logs, graphs, '
                             'metadata, and report (default pantheon/src/experiments/data)')
    parser.add_argument('--pkill-cleanup', action='store_true',
...
def setup_args(subparsers):
    ...
    parser_local_test = test_subparsers.add_parser('local', help='test schemes locally in mahimahi emulated networks')

    parser_remote_test = test_subparsers.add_parser('remote',
                                                    help='test schemes between local and remote in real-life networks')
    parser_remote_test.add_argument('remote_path', metavar='HOST:PANTHEON-DIR',
                                    help='HOST ([user@]IP) and PANTHEON-DIR (remote pantheon directory)')

    parse_test_shared(parser_local_test)
    parse_test_shared(parser_remote_test)

    # Arguments for local only
    parser_local_test.add_argument('--uplink-trace', metavar='TRACE',
    ...
    parser_local_test.add_argument('--downlink-trace', metavar='TRACE',
                                   default=Path(context.src_dir) / 'experiments' / '12mbps.trace',
                                   help='downlink trace (from receiver to sender) to pass to mm-link '
                                        '(default pantheon/test/12mbps.trace)')
    parser_local_test.add_argument('--prepend-mm-cmds', metavar='"CMD1 CMD2..."',
                                   help='mahimahi shells to run outside of mm-link')
    parser_local_test.add_argument('--append-mm-cmds', metavar='"CMD1 CMD2..."',
                                   help='mahimahi shells to run inside of mm-link')
    parser_local_test.add_argument('--extra-mm-link-args', metavar='"ARG1 ARG2..."',
    ...
    # Arguments for remote only
    parser_remote_test.add_argument('--sender', choices=['local', 'remote'], default='local', action='store',
                                    dest='sender_side', help='the side to be data sender (default local)')
    parser_remote_test.add_argument('--tunnel-server', choices=['local', 'remote'], default='remote', action='store',
                                    dest='server_side',
                                    help='the side to run pantheon tunnel server on (default remote)')
    parser_remote_test.add_argument('--local-addr', metavar='IP',
                                    help='local IP address that can be reached from remote host, '
                                         'required if "--tunnel-server local" is given')
    parser_remote_test.add_argument('--local-if', metavar='INTERFACE', help='local interface to run pantheon tunnel on')
    parser_remote_test.add_argument('--remote-if', metavar='INTERFACE',
