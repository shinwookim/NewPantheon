...
class Test(object):
    def __init__(self, args, run_id, cc):
        ...
        self.run_id = run_id
        self.cc = cc
        self.data_dir = Path(args.data_dir).resolve().absolute()
        ...
        if self.test_config is not None:
            ...
            for flow in args.test_config['flows']:
                ...
                local_p = Path(context.src_dir) / 'wrappers' / (cc + '.py')
                remote_p = Path(cc_src_remote_dir) / 'wrappers' / (cc + '.py')
                self.flow_objs[tun_id] = Flow(cc=cc, cc_src_local=local_p, cc_src_remote=remote_p, run_first=run_first,
    ...
    def setup_mm_cmd(self):
        ...
        self.mm_datalink_log = self.data_dir / mm_datalink_log
        self.mm_acklink_log = self.data_dir / mm_acklink_log
    ...
    def prepare_tunnel_log_paths(self):
        ...
        for tun_id in range(1, self.flows + 1):
            ...
            datalink_ingress_logname = ('%s_flow%s_uid%s.log.ingress' % (self.datalink_name, tun_id, uid))
            self.datalink_ingress_logs[tun_id] = Path(local_tmp) / datalink_ingress_logname

            datalink_egress_logname = ('%s_flow%s_uid%s.log.egress' % (self.datalink_name, tun_id, uid))
            self.datalink_egress_logs[tun_id] = Path(local_tmp) / datalink_egress_logname

            acklink_ingress_logname = ('%s_flow%s_uid%s.log.ingress' % (self.acklink_name, tun_id, uid))
            self.acklink_ingress_logs[tun_id] = Path(local_tmp) / acklink_ingress_logname

            acklink_egress_logname = ('%s_flow%s_uid%s.log.egress' % (self.acklink_name, tun_id, uid))
            self.acklink_egress_logs[tun_id] = Path(local_tmp) / acklink_egress_logname

            if self.mode == 'remote':
                if self.sender_side == 'local':
                    self.datalink_ingress_logs[tun_id] = Path(remote_tmp) / datalink_ingress_logname
                    self.acklink_egress_logs[tun_id] = Path(remote_tmp) / acklink_egress_logname
                else:
                    self.datalink_egress_logs[tun_id] = Path(remote_tmp) / datalink_egress_logname
                    self.acklink_ingress_logs[tun_id] = Path(remote_tmp) / acklink_ingress_logname
    ...
    def setup(self):
        ...
        if self.mode == 'local':
            ...
            if self.ntp_addr is not None:
                self.local_ofst, self.remote_ofst = utils.query_clock_offset(self.ntp_addr, self.r['ssh_cmd'])
    ...
    def process_tunnel_logs(self):
        ...
        merge_tunnel_logs = path.join(context.src_dir, 'experiments', 'merge_tunnel_logs.py')
        ...
        for tun_id in range(1, self.flows + 1):
            ...
            uid = uuid.uuid4()
            datalink_tun_log = path.join(utils.tmp_dir,
                                         '%s_flow%s_uid%s.log.merged' % (self.datalink_name, tun_id, uid))
            acklink_tun_log = path.join(utils.tmp_dir, '%s_flow%s_uid%s.log.merged' % (self.acklink_name, tun_id, uid))

            cmd = [merge_tunnel_logs, 'single', '-i', self.datalink_ingress_logs[tun_id], '-e',
                   self.datalink_egress_logs[tun_id], '-o', datalink_tun_log]
            if apply_ofst:
                cmd += ['-i-clock-offset', data_i_ofst, '-e-clock-offset', data_e_ofst]
            call(cmd)

            cmd = [merge_tunnel_logs, 'single', '-i', self.acklink_ingress_logs[tun_id], '-e',
                   self.acklink_egress_logs[tun_id], '-o', acklink_tun_log]
            if apply_ofst:
                cmd += ['-i-clock-offset', ack_i_ofst, '-e-clock-offset', ack_e_ofst]
            call(cmd)
    ...
    def record_time_stats(self):
        stats_log = path.join(self.data_dir, '%s_stats_run%s.log' % (self.cc, self.run_id))
    ...
    def run(self):
        msg = 'Testing scheme %s for experiment run %d/%d...' % (self.cc, self.run_id, self.run_times)
