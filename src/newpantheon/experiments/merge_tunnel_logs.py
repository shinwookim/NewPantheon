...
def parse_arguments():
    ...
    subparsers = parser.add_subparsers(help="single or multiple mode", dest="mode")

    # subparser for single mode
    single_parser = subparsers.add_parser(
        "single",
        help="merge the ingress log and egress log of a single "
    ...
    while send_l or recv_l:
        ...
        if (send_l and recv_l and send_ts_cal <= recv_ts_cal) or not recv_l:
            ...
            if recv_uid in send_pkts:
                ...
                # nonexistent packet
                sys.exit(
                    "Warning: received a packet with nonexistent " "uid %s\n" % recv_uid
                )
